    ...
    W: AsyncWrite + Unpin,
{
    pub(crate) async fn accept(
        ...
        frame_writer: impl Into<SnellStreamWriter<W>>,
    ) -> Result<Self> {
        let mut frame_writer = frame_writer.into();
        frame_writer.write_empty_tunnel_reply().await?;
        Ok(Self::from_parts(reader.into(), frame_writer))
    }
...
mod tests {
    ...
    async fn udp_client_open_writes_udp_request_and_accepts_empty_tunnel() {
        ...
        let client = async {
            UdpClientStream::open_io(client_download, client_upload, psk, VERSION_4)
                .await
                .unwrap()
        };
    ...
    async fn udp_client_open_supports_v6_stream() {
        ...
        let client = async {
            UdpClientStream::open_io(client_download, client_upload, psk, VERSION_6)
                .await
                .unwrap()
        };

        let server = async {
            let mut reader = SnellStreamReader::new(server_upload, psk, VERSION_6).unwrap();
            let request = reader.read_client_request().await.unwrap();
            assert_eq!(
                request,
                ClientRequest::Udp {
            ...
            let writer = SnellStreamWriter::new(server_download, psk, VERSION_6).unwrap();
            UdpServerStream::accept(reader, writer).await.unwrap()
        };
    ...
    async fn udp_client_open_rejects_non_empty_tunnel_reply() {
        ...
        let client = async {
            UdpClientStream::open_io(client_download, client_upload, psk, VERSION_4).await
        };
