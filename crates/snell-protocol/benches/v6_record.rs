...
fn decode_one(decoder: &mut V6UnshapedDecoder, buf: &mut Buffer, wire: &[u8]) -> usize {
...
fn decode_one_shaped(decoder: &mut V6ShapedDecoder, buf: &mut Buffer, wire: &[u8]) -> usize {
    buf.extend_from_slice(wire).unwrap();
    let mut decoded = 0usize;
    loop {
        match decoder.decode(buf).unwrap() {
