...
async fn snell_stream_v6_reads_tcp_request_and_payload_frames() {
    ...
    let read = async {
        let mut reader = SnellStreamReader::new(server, psk, crate::ProtocolVersion::V6).unwrap();
...
async fn snell_stream_v6_reads_tunnel_reply_tail_and_zero_chunk() {
    ...
    let read = async {
        let mut reader = SnellStreamReader::new(client, psk, crate::ProtocolVersion::V6).unwrap();
...
async fn v6_udp_packet_splits_across_chunk_records_and_advances_chunk_state() {
    ...
    let read = async {
        let mut reader = V6StreamReader::new(server, psk).unwrap();
        let mut records = 0;
