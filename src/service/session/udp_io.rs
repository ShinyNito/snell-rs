...
use crate::service::outbound::udp::ensure_full_datagram_sent;
...
pub(crate) const MAX_SOCKS_UDP_HEADER: usize = SOCKS_UDP_RSV_FRAG_LEN
    ...
    + u8::MAX as usize
    + SOCKS_UDP_PORT_LEN;
pub(crate) const MAX_VALID_SOCKS_UDP_DATAGRAM: usize = MAX_PACKET_SIZE + SOCKS_UDP_RSV_FRAG_LEN;
const SOCKS_UDP_RECV_BUFFER_LEN: usize = MAX_VALID_SOCKS_UDP_DATAGRAM + 1;
const SOCKS_UDP_RECV_RETAIN_CAPACITY: usize = SOCKS_UDP_RECV_BUFFER_LEN * 2;
...
pub(crate) async fn recv_socks_udp_datagram_into(
    ...
    if datagram.capacity() > SOCKS_UDP_RECV_RETAIN_CAPACITY {
        *datagram = BytesMut::with_capacity(SOCKS_UDP_RECV_BUFFER_LEN);
    }

    datagram.reserve(SOCKS_UDP_RECV_BUFFER_LEN);

    let (n, peer) = socket.recv_buf_from(datagram).await?;
    if socks_udp_datagram_too_large(n) {
        datagram.clear();
...
fn socks_udp_datagram_too_large(n: usize) -> bool {
    n > MAX_VALID_SOCKS_UDP_DATAGRAM
}
...
mod tests {
    ...
    fn reframe(mut datagram: BytesMut, kind: SnellUdpPacketKind) -> (BytesMut, usize) {
        ...
        let prefix_start = reframe_socks_udp_packet(&mut datagram, &header, kind).unwrap();
    ...
    fn rejects_reframed_payload_larger_than_snell_packet() {
        ...
        assert!(matches!(
            reframe_socks_udp_packet(&mut datagram, &header, SnellUdpPacketKind::Request),
    ...
    async fn recv_socks_udp_datagram_reports_peer_and_preserves_bytes() {
        ...
        let (n, peer) = recv_socks_udp_datagram_into(&receiver, &mut received)
            .await
            .unwrap();
    ...
    fn recv_oversized_sentinel_marks_datagram_too_large() {
        assert!(!udp_io::socks_udp_datagram_too_large(
            udp_io::MAX_VALID_SOCKS_UDP_DATAGRAM
        ));
        assert!(udp_io::socks_udp_datagram_too_large(
            udp_io::SOCKS_UDP_RECV_BUFFER_LEN
        ));
    }
